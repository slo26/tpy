...
class MonthlyDetailPurchaseBillStatementForm extends ConfigFormBase {
    ...
    public function buildForm(array $form, FormStateInterface $form_state, $vendor_nid = NULL, $begin_date = NULL , $end_date = NULL) {
        ...
        $section_two = '<table id="monthly-detail-purchase-bill-s2"><tbody><tr><td align="left">日期起迄區間: ' . $begin_date . " ~ " . $end_date . '</td><td width="250px">頁次: 1</td></tr>';
        $section_two .= '<tr id="monthly-detail-purchase-bill-s2-cust"><td>廠商名稱: <a href="/node/' . $vendor_nid .'/edit" target="_blank">' . $vendor->field_vendor_name->value . '</a></td><td>製表日期: ' . date("Y/m/d") . '</td></tr>';
        ...
        $section_three = '<table id="monthly-detail-purchase-bill-s3"><tbody><tr><th width="100">帳款日期</th><th width="150">採購單憑證</th><th width="150">物品編號</th><th width="360">物品名稱</th><th width="50">數量</th><th width="30">單位</th><th width="110">稅後單價</th><th width="110">稅後售價</th><th width="50">種類</th></tr>';
